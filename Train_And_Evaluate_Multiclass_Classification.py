"""
Train_And_Evaluate_Multiclass_Classification.py — Filter, Split, Train, and Evaluate (Multiclass, 11 classes)

Takes a raw Final_Labels configuration CSV (Id_Number, Patient Question, RLP_Label,
MLP_Label, golden_labels), filters out ambiguous-label rows, extracts a single
"dominant" label per row (the first label listed) from the chosen label column,
fine-tunes an 11-class (10 CDs + "No Distortion") MentalBERT/MentalRoBERTa
classifier across multiple seeds, and saves self-describing checkpoints
(config.json carries id2label/label2id) ready for inference.

WHY 11 CLASSES, NOT 10 + ABSENCE:
Multilabel drops "No Distortion" from its label space because independent sigmoid
outputs per label could otherwise produce a contradiction (e.g. predicting both
"No Distortion" and "Overgeneralization" as present at once). Multiclass uses
softmax + argmax, which is inherently mutually exclusive — exactly one label wins,
so that contradiction can never happen regardless of whether "No Distortion" is one
of the classes. Here "No Distortion" is kept as an explicit 11th class so the model
is a self-contained classifier: given any input, it picks the single most likely
label out of all 11, including "no distortion" as a valid answer. (A 10-class
version would only make sense as the second stage of a pipeline behind a separate
binary distortion detector — that is NOT what this script does.)

NOTE: the paper does NOT train multiclass on MLP labels. RLP has at most two
labels per row, so taking the first as the "dominant" label is a reasonably
safe approximation. MLP can carry many more labels per row, so picking the
first one would be a much weaker/more arbitrary approximation of "the one
dominant distortion" — there's no guarantee the first-listed MLP label is
actually the most prominent one when there are several. mlp is kept as a
valid --label_type choice here for flexibility/consistency with the binary
and multilabel scripts, but running multiclass on mlp does NOT match the
paper's methodology and results reported there.
...
SPLIT METHOD:
  fixed_ids (default) Reproduces the paper's exact train/dev/test assignment via the
                       ...
                       configs (gpt4-0.5/0.7, gpt4o-0.5/0.7) so cross-config
                       comparison is meaningful — but the row COUNT in each split
                       still differs per config, because each config's own
                       ambiguous-label filtering removes a different subset of rows
...
Example usage:
  ...
  Full paper-matching run (5 seeds, fixed split):
    python Train_And_Evaluate_Multiclass_Classification.py \
...
parser = argparse.ArgumentParser(
    description="Filter, split, train, and evaluate an 11-class cognitive distortion classifier"
)
...
parser.add_argument('--negative_label', type=str, default='No Distortion',
                     help='Name of the "no distortion" class, kept as an explicit 11th class (default: "No Distortion")')
...
parser.add_argument('--seeds', type=int, nargs='+', default=[42, 123, 456, 789, 1024],
                     help='Model-training seeds (default: the paper\'s 5 seeds, to average out '
                          'training-run variance). Pass one value, e.g. --seeds 42, for a quick '
                          'single-run test. Same split used for every seed.')
...
# Fixed 11-class taxonomy (10 CDs + the explicit negative class). Kept constant
# rather than derived per-config so every run produces a model with the same
# label space and ordering, regardless of which classes happen to survive
# filtering for a given config — LabelEncoder fit on a config's train split
# alone risks a different class count/order per config, or a hard crash if
# val/test contains a label never seen in that config's training split.
...
ALL_LABELS = sorted(COGNITIVE_DISTORTION_LABELS + [args.negative_label])
...
def plot_confusion_matrices(y_true_val, y_pred_val, y_true_test, y_pred_test, run_id, seed, class_labels):
    fig, axes = plt.subplots(1, 2, figsize=(15, 6))
    fig.suptitle(f"Confusion Matrices — {run_id} (Seed {seed})")

    ConfusionMatrixDisplay(
        confusion_matrix=confusion_matrix(y_true_val, y_pred_val, labels=list(range(len(class_labels)))),
    ...
    ConfusionMatrixDisplay(
        confusion_matrix=confusion_matrix(y_true_test, y_pred_test, labels=list(range(len(class_labels)))),
        display_labels=class_labels
...
# Dominant label = first label listed in the chosen column. Unlike multilabel,
# "No Distortion" is NOT dropped here — it stays as a valid dominant label.
source_column = LABEL_COLUMN_MAP[args.label_type]
if args.label_type == 'mlp':
    print(f"[{run_id}] WARNING: --label_type mlp was NOT used for multiclass in the paper "
          f"(MLP rows can carry many labels, making 'first label = dominant' a weak "
          f"approximation). Proceeding anyway, but these results are not paper-comparable.")
data['dominant_label'] = data[source_column].apply(parse_labels).apply(lambda x: x[0] if x else None)

before_count = len(data)
data = data[data['dominant_label'].notna()].reset_index(drop=True)
...
unknown_mask = ~data['dominant_label'].isin(ALL_LABELS)
if unknown_mask.any():
    unknown_values = sorted(data.loc[unknown_mask, 'dominant_label'].unique())
    print(f"[{run_id}] WARNING: dropping {unknown_mask.sum()} rows with dominant label(s) outside the known taxonomy: {unknown_values}")
...
# ---------------------------------------------------------------------------
# Encode labels using the fixed 11-class taxonomy (no sklearn LabelEncoder —
# avoids a crash on any label unseen in this config's training split, and
# guarantees identical class ordering across all configs/label_types)
# ---------------------------------------------------------------------------
observed_labels = sorted(set(train_data['dominant_label']) | set(val_data['dominant_label']) | set(test_data['dominant_label']))
missing = set(ALL_LABELS) - set(observed_labels)
if missing:
    print(f"[{run_id}] WARNING: expected labels never seen in this config's filtered data: {missing}")
...
for split_df in (train_data, val_data, test_data):
    split_df['label_id'] = split_df['dominant_label'].map(label2id).astype(int)

y_true_val = val_data['label_id'].values
y_true_test = test_data['label_id'].values
...
train_dataset = PreTokenizedMentalHealthDataset(
    train_encodings, train_data['label_id'].values, train_data['Id_Number'].astype(int).values
)
val_dataset = PreTokenizedMentalHealthDataset(
    val_encodings, val_data['label_id'].values, val_data['Id_Number'].astype(int).values
)
test_dataset = PreTokenizedMentalHealthDataset(
    test_encodings, test_data['label_id'].values, test_data['Id_Number'].astype(int).values
)
...
for seed in SEEDS:
    ...
    with open(results_file, "a") as f:
        ...
        f.write(classification_report(y_true_val, y_pred_val, target_names=label_names,
                                       labels=list(range(len(label_names))), digits=4) + "\n")

    classification_report_dict = classification_report(
        y_true_val, y_pred_val, target_names=label_names,
        labels=list(range(len(label_names))), digits=4, output_dict=True
    )
    ...
    plot_confusion_matrices(y_true_val, y_pred_val, y_true_test, y_pred_test, run_id, seed, label_names)
