"""
predict_multiclass.py — Run inference with a trained multiclass cognitive distortion model

Loads a checkpoint produced by Train_And_Evaluate_Multiclass_Classification.py (e.g.
results/results_<run_id>_best/<run_id>_seed_<seed>/) and predicts a single dominant
cognitive distortion label (or "No Distortion") for new, unlabeled text. The
checkpoint is self-describing — label names come from the model's own config.json
(id2label), cross-checked against label_classes.json in the same folder if present —
so no separate label list needs to be passed in.

Note: this is an 11-class classifier (10 CDs + "No Distortion" as an explicit class),
matching the paper's methodology — see Train_And_Evaluate_Multiclass.py docstring for
why multiclass keeps "No Distortion" explicit while multilabel does not.
...
parser = argparse.ArgumentParser(description="Run multiclass cognitive distortion inference on new data")
